...
describe('Assert Action', () => {
    ...
    describe('assert primitive action', () => {
        ...
        it('should fail when eq comparison is false', async () => {
            ...
            await expect(engine.executePrimitive(action, context, new Map())).rejects.toThrow(/assert failed.*actual=42.*expected=99.*op=eq/);
        });
        it('should include custom message on failure', async () => {
            ...
            await expect(engine.executePrimitive(action, context, new Map())).rejects.toThrow(/assert failed: balance mismatch.*actual=10.*expected=20.*op=eq/);
        });
        ...
        it('should fail neq comparison when values are equal', async () => {
            ...
            await expect(engine.executePrimitive(action, context, new Map())).rejects.toThrow(/assert failed.*op=neq/);
        });
        ...
        it('should fail gte when less', async () => {
            ...
            await expect(engine.executePrimitive(action, context, new Map())).rejects.toThrow(/assert failed.*op=gte/);
        });
        ...
        it('should fail lt when greater', async () => {
            ...
            await expect(engine.executePrimitive(action, context, new Map())).rejects.toThrow(/assert failed.*op=lt/);
        });
        ...
        it('should fail lte when greater', async () => {
            ...
            await expect(engine.executePrimitive(action, context, new Map())).rejects.toThrow(/assert failed.*op=lte/);
        });
        ...
        it('should fail gt when less', async () => {
            ...
            await expect(engine.executePrimitive(action, context, new Map())).rejects.toThrow(/assert failed.*op=gt/);
        });
        it('should use `to` + `signature` form (call resolver)', async () => {
            const mockResolver = {
                resolve: jest.fn(async (value, ctx, scope) => {
                    if (value.type === 'call') {
            ...
            await engine.executePrimitive(action, context, new Map());
            expect(mockResolver.resolve).toHaveBeenCalledWith({ type: 'call', arguments: { to: '0xSomeProxyAddress', signature: 'depositManager() returns (address)', values: [] } }, context, new Map());
        ...
        it('should use `actual` form with read-balance resolver', async () => {
            ...
            await engine.executePrimitive(action, context, new Map());
            expect(mockResolver.resolve).toHaveBeenCalledWith({ type: 'read-balance', arguments: { address: '0xDeployer' } }, context, new Map());
        ...
        it('should not store outputs when action has custom output', async () => {
            ...
            await engine.executePrimitive(action, context, new Map(), true);
            const setOutputCalls = context.setOutput.mock.calls;
            const actualOutputs = setOutputCalls.filter((c) => c[0].includes('.actual'));
        ...
        it('should fail when no comparator key is provided', async () => {
            ...
            await expect(engine.executePrimitive(action, context, new Map())).rejects.toThrow(/assert must have exactly one of/);
        });
        it('should fail when more than one comparator key is provided', async () => {
            ...
            await expect(engine.executePrimitive(action, context, new Map())).rejects.toThrow(/assert must have exactly one comparator, but got: eq, gte/);
        });
        ...
        it('should describe call form as signature in error message', async () => {
            ...
            await expect(engine.executePrimitive(action, context, new Map())).rejects.toThrow(/assert failed.*getOwner\(\) returns \(address\)/);
        });
    });
});
//# sourceMappingURL=assert-action.spec.js.map