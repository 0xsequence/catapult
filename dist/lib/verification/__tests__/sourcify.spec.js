...
describe('Sourcify Verification Platform', () => {
    ...
    let mockNetwork;
    beforeEach(() => {
        platform = new sourcify_1.SourcifyVerificationPlatform();
        mockNetwork = {
    ...
    describe('isContractAlreadyVerified', () => {
        it('should return true for verified contract', async () => {
            ...
            const result = await platform.isContractAlreadyVerified('0x1234567890123456789012345678901234567890', mockNetwork);
            expect(result).toBe(true);
            expect(mockFetch).toHaveBeenCalledWith('https://sourcify.dev/server/check-by-addresses?addresses=0x1234567890123456789012345678901234567890&chainIds=1', expect.objectContaining({
                method: 'GET',
        ...
        it('should treat 409 partial already verified as success (notice)', async () => {
            ...
            mockFetch.mockResolvedValueOnce({
                ...
                text: jest.fn().mockResolvedValue(JSON.stringify({
                    error: 'The contract 0x... on chainId 1 is already partially verified. The provided new source code also yielded a partial match and will not be stored unless it\'s a full match',
                    message: 'The contract 0x... on chainId 1 is already partially verified. The provided new source code also yielded a partial match and will not be stored unless it\'s a full match'
                }))
            });
            const result = await platform.verifyContract(mockRequest);
            expect(result.success).toBe(true);
...
//# sourceMappingURL=sourcify.spec.js.map