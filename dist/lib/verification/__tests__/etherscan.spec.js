...
describe('Etherscan Verification', () => {
    ...
    describe('submitVerification', () => {
        it('should submit verification successfully', async () => {
            ...
            const result = await (0, etherscan_1.submitVerification)({
            ...
            expect(mockedFetch).toHaveBeenCalledWith('https://api.etherscan.io/v2/api?chainid=1', expect.objectContaining({
                method: 'POST',
        ...
        it('should include constructor arguments when provided', async () => {
            ...
            mockedFetch.mockResolvedValueOnce(createMockResponse(mockResponseData));
            await (0, etherscan_1.submitVerification)({
                address: '0x742d35Cc6596C743B2c8d12Cd84d5B8FbA4F3C',
                buildInfo: mockBuildInfo,
                contract: mockContract,
        ...
        it('should use correct API URL for different networks', async () => {
            ...
            expect(mockedFetch).toHaveBeenCalledWith('https://api.etherscan.io/v2/api?chainid=11155111', expect.any(Object));
        });
        it('should use v2 API format for any network', async () => {
            ...
            expect(mockedFetch).toHaveBeenCalledWith('https://api.etherscan.io/v2/api?chainid=999999', expect.any(Object));
        });
    ...
    describe('checkVerificationStatus', () => {
        it('should return success status', async () => {
            ...
            mockedFetch.mockResolvedValueOnce(createMockResponse(mockResponseData));
            const status = await (0, etherscan_1.checkVerificationStatus)('test-guid', 'test-api-key', mockNetwork);
    ...
    describe('waitForVerification', () => {
        it('should return when verification completes successfully', async () => {
            ...
            const status = await (0, etherscan_1.waitForVerification)('test-guid', 'test-api-key', mockNetwork, 10000);
            expect(status.isComplete).toBe(true);
            expect(status.isSuccess).toBe(true);
            expect(mockedFetch).toHaveBeenCalledTimes(2);
        });
        it('should timeout after specified duration', async () => {
            ...
            await expect((0, etherscan_1.waitForVerification)('test-guid', 'test-api-key', mockNetwork, 1000)).rejects.toThrow('Verification timed out after 1 seconds');
            expect(mockedFetch).toHaveBeenCalledTimes(1);
        });
    });
    describe('isContractAlreadyVerified', () => {
        ...
        it('should return true when contract is verified', async () => {
            ...
            const result = await (0, etherscan_1.isContractAlreadyVerified)(testAddress, testApiKey, mockNetwork);
            expect(result).toBe(true);
            expect(mockedFetch).toHaveBeenCalledWith(expect.stringContaining('chainid=1'), expect.objectContaining({
                method: 'GET'
            }));
        });
        ...
        it('should return false and log warning when API request fails', async () => {
            ...
            const result = await (0, etherscan_1.isContractAlreadyVerified)(testAddress, testApiKey, mockNetwork);
            expect(result).toBe(false);
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining(`Failed to check verification status for ${testAddress}`));
        ...
        it('should return false when HTTP response is not ok', async () => {
            ...
            const result = await (0, etherscan_1.isContractAlreadyVerified)(testAddress, testApiKey, mockNetwork);
            expect(result).toBe(false);
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to check verification status'));
            consoleSpy.mockRestore();
        });
        it('should use correct chainId in API call for different networks', async () => {
            ...
            await (0, etherscan_1.isContractAlreadyVerified)(testAddress, testApiKey, arbitrumNetwork);
            expect(mockedFetch).toHaveBeenCalledWith(expect.stringContaining('chainid=42161'), expect.any(Object));
        });
    });
});
//# sourceMappingURL=etherscan.spec.js.map